...
export class MisPublicacionesComponent implements OnInit, OnDestroy {
  ...
  actualizarLista(): void {
    ...
    const filtrados = this.misPosts.filter(
      (p) =>
    ...
    this.totalPaginas = Math.max(
      1,
      Math.ceil(filtrados.length / this.cantidadMostrar)
    );
  ...
  publicarPost(): void {
    ...
    const obs =
      this.modoImagen === 'archivo' && this.imagenSeleccionada
        ? this.postService.createPostImage(
            title,
            description,
            userName,
